...
from ...core.models.auth import User, AuthProvider, AuthConfig
...
async def verify_admin_access(credentials: HTTPAuthorizationCredentials = Depends(security)) -> User:
...
async def get_system_metrics(admin_user: User = Depends(verify_admin_access)) -> SystemMetrics:
    ...
    try:
        ...
        response_time = await metrics_service.get_avg_response_time(start_time, end_time)
        total_cost = await metrics_service.get_total_cost(start_time, end_time)
        tokens_processed = await metrics_service.get_tokens_processed(start_time, end_time)
...
async def get_agent_conversations(
    ...
    try:
        ...
        for conv in conversations:
            response_data.append(AgentConversationResponse(
                id=conv.id,
        ...
        logger.info("Agent conversations retrieved", {
            "admin_user": admin_user.email,
...
async def get_auth_config(admin_user: User = Depends(verify_admin_access)) -> Dict[str, Any]:
...
async def get_integrations(admin_user: User = Depends(verify_admin_access)) -> List[IntegrationResponse]:
    ...
    try:
        ...
        for integration in integrations:
            response_data.append(IntegrationResponse(
                id=integration.id,
        ...
        logger.info("Integrations retrieved", {
            "admin_user": admin_user.email,
            "integration_count": len(response_data)
        })
...
async def get_system_health(admin_user: User = Depends(verify_admin_access)) -> Dict[str, Any]:

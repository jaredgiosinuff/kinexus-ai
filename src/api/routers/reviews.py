...
from api.dependencies import get_db, get_current_active_user, require_reviewer, create_audit_logger
from database.models import User, Review, ReviewStatus, Document
...
class ReviewRejection(BaseModel):
    ...
    feedback: str = Field(..., min_length=1, description="Feedback is required for rejection")
...
class ReviewCreate(BaseModel):
    ...
    ai_reasoning: Optional[str] = None
    ai_confidence: Optional[int] = Field(None, ge=1, le=100, description="AI confidence from 1-100")
    ai_model: Optional[str] = None
    change_context: Optional[Dict[str, Any]] = None
...
async def get_reviews(
    status: Optional[List[ReviewStatus]] = Query(None, description="Filter by review status"),
    reviewer_id: Optional[str] = Query(None, description="Filter by reviewer ID"),
    document_type: Optional[str] = Query(None, description="Filter by document type"),
    priority_min: Optional[int] = Query(None, ge=1, le=10, description="Minimum priority level"),
...
async def get_my_review_queue(
    status: Optional[List[ReviewStatus]] = Query([ReviewStatus.PENDING, ReviewStatus.IN_REVIEW]),

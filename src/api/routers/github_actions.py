...
class ChangeAnalysisRequest(BaseModel):
    ...
    scope: str = Field(..., description="Documentation scope: repository, internal, or enterprise")
...
class DocumentationUpdateRequest(BaseModel):
    ...
    targets: List[str] = Field(default_factory=list, description="Target documentation systems")
...
async def update_documentation(
    ...
    try:
        ...
        if request.scope == "repository":
            ...
            targets_updated.extend(["confluence", "sharepoint", "customer_docs", "api_portal"])
            changes_made.extend([
        ...
        quality_score = min(1.0, len(targets_updated) / max(1, len(request.targets or targets_updated)))
...
async def quality_check(
    ...
    try:
        ...
        quality_results = {
            ...
            "compliance_status": "compliant" if request.scope == "enterprise" else "not_applicable"
        }
        ...
        if request.scope == "enterprise":
            quality_results["compliance_checks"] = {
                ...
                "legal_review": "required" if request.impact_score > 8 else "not_required"
            }
...
async def _update_repository_docs(request: DocumentationUpdateRequest, db: Session) -> None:
...
async def _update_internal_docs(request: DocumentationUpdateRequest, db: Session) -> None:
    """Update internal documentation systems."""
...
async def _update_enterprise_docs(request: DocumentationUpdateRequest, db: Session) -> None:
    """Update enterprise documentation systems."""

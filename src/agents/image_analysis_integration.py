...
from .image_analysis_framework import (
    ImageAnalysisEngine, ImageAnalysisRequest, ImageAnalysisResult,
    ImageType, AnalysisTask, DocumentImageAnalyzer
)

# Import existing systems
from .self_corrective_rag import SelfCorrectiveRAG, RAGQuery, RAGTaskType
...
class CRAGImageEnhancer:
    ...
    def __init__(self, crag_system: SelfCorrectiveRAG,
                 image_analyzer: ImageAnalysisEngine):
        self.crag_system = crag_system
        self.image_analyzer = image_analyzer

    async def process_query_with_images(self, query: RAGQuery,
                                      image_context: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if image_context:
            ...
            image_results = await self.image_analyzer.batch_analyze_images(image_analysis_requests)

        # Enhance CRAG result with image insights
        enhanced_result = await self._enhance_with_image_insights(crag_result, image_results)

        return {
            'crag_result': crag_result,
            'image_analysis': image_results,
            'enhanced_response': enhanced_result,
            'combined_confidence': self._calculate_combined_confidence(crag_result, image_results)
        }
    ...
    async def _enhance_with_image_insights(self, crag_result, image_results) -> str:
        ...
        for result in image_results:
            ...
            if result.text_extraction and result.text_extraction.extracted_text:
                insights.append(f"Image contains text: {result.text_extraction.extracted_text[:100]}...")

            if result.quality_assessment:
                if result.quality_assessment.overall_score < 0.7:
                    insights.append(f"Image quality issues detected: {', '.join(result.quality_assessment.issues_detected)}")

            if result.content_validation and not result.content_validation.is_valid:
                insights.append(f"Content validation failed: {', '.join(result.content_validation.missing_elements)}")
        ...
        if image_insights:
            ...
            try:
                enhanced_response = await self._call_bedrock_for_enhancement(enhancement_prompt)
    ...
    def _calculate_combined_confidence(self, crag_result, image_results) -> float:
        ...
        if image_results:
            image_confidences = [r.confidence for r in image_results if r.confidence > 0]
            avg_image_confidence = sum(image_confidences) / len(image_confidences) if image_confidences else 0.5
...
class DocumentationImageValidator:
    ...
    def __init__(self, image_analyzer: ImageAnalysisEngine,
    ...
    async def validate_document_images(self, document_id: str,
                                     validation_config: Dict[str, Any] = None) -> ImageAnalysisReport:
        ...
        for img_ref in image_references:
            request = ImageAnalysisRequest(
                ...
                context={
                    ...
                    'expected_content': self._extract_expected_content(img_ref, document)
                }
            )
            validation_requests.append(request)

        # Perform batch analysis
        analysis_results = await self.image_analyzer.batch_analyze_images(validation_requests)
    ...
    def _extract_image_references(self, document: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _classify_image_from_context(self, alt_text: str, context: str) -> str:
        ...
        if any(word in alt_lower for word in ['screenshot', 'screen shot', 'capture']):
            ...
            return 'chart_graph'
        elif any(word in alt_lower for word in ['mockup', 'wireframe', 'ui', 'interface']):
            return 'ui_mockup'
        else:
    ...
    def _extract_expected_content(self, img_ref: Dict[str, Any],
        ...
        if 'api' in context.lower():
            expected_content['required_text'] = ['API', 'endpoint', 'request', 'response']
    ...
    async def _generate_validation_report(self, document_id: str,
                                        analysis_results: List[ImageAnalysisResult],
                                        start_time: datetime) -> ImageAnalysisReport:
        ...
        for result in analysis_results:
            ...
            if result.quality_assessment:
                ...
                all_issues.extend([
                    {'type': 'quality', 'issue': issue, 'image_id': result.request_id}
        ...
        accessibility_compliance = sum(accessibility_scores) / len(accessibility_scores) if accessibility_scores else 1.0
        content_accuracy = sum(content_scores) / len(content_scores) if content_scores else 1.0
        ...
        if accessibility_compliance < 0.8:
            recommendations.add("Enhance accessibility by improving color contrast and text size")

        if content_accuracy < 0.8:
            recommendations.add("Verify that images contain all necessary documentation elements")
    ...
    async def _record_validation_metrics(self, document_id: str,
...
class WebhookImageProcessor:
    ...
    def __init__(self, image_validator: DocumentationImageValidator,
                 crag_enhancer: CRAGImageEnhancer):
        self.image_validator = image_validator
        self.crag_enhancer = crag_enhancer

    async def process_documentation_update(self, webhook_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        image_files = [f for f in modified_files if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.svg'))]
...
class ImageAnalysisIntegrationManager:
    ...
    async def validate_all_documentation_images(self) -> Dict[str, Any]:
        ...
        avg_quality = sum(r.overall_quality_score for r in validation_reports) / len(validation_reports) if validation_reports else 1.0
        avg_accessibility = sum(r.accessibility_compliance for r in validation_reports) / len(validation_reports) if validation_reports else 1.0
        ...
        return {
            ...
            'recommendations': self._generate_global_recommendations(validation_reports)
        }

    def _generate_global_recommendations(self, reports: List[ImageAnalysisReport]) -> List[str]:
        ...
        if sum(accessibility_scores) / len(accessibility_scores) < 0.8:
            recommendations.add("Establish accessibility guidelines for documentation images")
        ...
        if issue_types.get('quality', 0) > len(reports) * 0.3:
            recommendations.add("Provide training on documentation image quality standards")

        if issue_types.get('accessibility', 0) > len(reports) * 0.2:
            recommendations.add("Implement automated accessibility checks in documentation workflow")
        ...
        try:
            ...
            return {
                ...
                'components': {
                    ...
                    'crag_enhancer': 'operational' if self.crag_enhancer else 'not_configured',
...
async def main():
    ...
    try:
        report = await integration_manager.image_validator.validate_document_images("doc-123")

...
from .self_corrective_rag import SelfCorrectiveRAG, CRAGResult, QualityMetric
from .agentic_rag_system import AgenticRAGSystem, RAGQuery, RAGTaskType, RetrievalStrategy
from .multi_agent_supervisor import MultiAgentSupervisor, AgentRole
...
class CRAGDocumentProcessor:
    ...
    def __init__(self, crag_system: SelfCorrectiveRAG, document_service: DocumentService,
                 metrics_service: MetricsService, config: CRAGConfiguration):
        self.crag_system = crag_system
        self.document_service = document_service
        self.metrics_service = metrics_service
        self.config = config

    async def process_document_query(self, query_text: str, document_context: Dict[str, Any],
                                   task_type: RAGTaskType = RAGTaskType.DOCUMENT_SEARCH) -> CRAGResult:
    ...
    async def process_document_update_suggestions(self, document_id: str,
                                                changes: List[Dict[str, Any]]) -> List[CRAGResult]:
        ...
        for change in changes:
            ...
            context = {
                ...
                'current_content': document.get('content', '')[:1000]  # First 1k chars for context
            }
    ...
    async def _collect_metrics(self, query: RAGQuery, result: CRAGResult,
        ...
        metrics_data = {
            ...
            'quality_scores': {
                metric.value: score for metric, score in result.quality_assessment.scores.items()
            },
    ...
    async def _store_learning_feedback(self, query: RAGQuery, result: CRAGResult):
        ...
        feedback_data = {
            ...
            'processing_efficiency': result.improvement_score / result.total_processing_time if result.total_processing_time > 0 else 0,
...
class CRAGWebhookProcessor:
    ...
    def __init__(self, crag_processor: CRAGDocumentProcessor,
                 agent_supervisor: MultiAgentSupervisor):
        self.crag_processor = crag_processor
        self.agent_supervisor = agent_supervisor

    async def process_github_webhook(self, webhook_data: Dict[str, Any]) -> List[CRAGResult]:
    ...
    async def process_jira_webhook(self, webhook_data: Dict[str, Any]) -> CRAGResult:
        ...
        query_text = f"What documentation should be updated for this {issue_type}: {summary}"
...
class CRAGPerformanceAnalyzer:
    ...
    def _perform_analysis(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis = {
            'performance_stats': {
                ...
                'success_rate': len([s for s in improvement_scores if s > 0]) / len(improvement_scores)
            },
    ...
    def _analyze_correction_patterns(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for metric in metrics:
            ...
            for correction in corrections:
                correction_frequency[correction] = correction_frequency.get(correction, 0) + 1
        ...
        return {
            'most_common_corrections': sorted(correction_frequency.items(),
                                            key=lambda x: x[1], reverse=True)[:5],
            'most_effective_corrections': sorted(avg_effectiveness.items(),
                                               key=lambda x: x[1], reverse=True)[:5],
    ...
    def _analyze_quality_trends(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_quality_improvement': sum(m.get('improvement_score', 0) for m in metrics) / len(metrics)
        }
    ...
    def _analyze_efficiency(self, metrics: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'efficiency_distribution': {
                ...
                'medium_efficiency': len([e for e in efficiency_scores if 0.05 <= e <= 0.1]),
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if avg_time > 10:
            recommendations.append("Consider optimizing retrieval pipeline - average processing time is high")

        # Iteration recommendations
        avg_iterations = performance.get('avg_iterations', 0)
        if avg_iterations > 2:
            recommendations.append("High correction iterations detected - consider improving initial retrieval quality")

        # Success rate recommendations
        success_rate = performance.get('success_rate', 0)
        if success_rate < 0.7:
            recommendations.append("Low success rate - review correction strategies and quality thresholds")

        # Correction pattern recommendations
        most_common = patterns.get('most_common_corrections', [])
        if most_common and most_common[0][1] > len(analysis.get('quality_trends', {}).get('quality_trend', [])) * 0.5:
            correction_type = most_common[0][0]
            recommendations.append(f"Frequent '{correction_type}' corrections suggest systematic issue - investigate root cause")

        # Efficiency recommendations
        avg_efficiency = efficiency.get('avg_efficiency', 0)
        if avg_efficiency < 0.05:
            recommendations.append("Low efficiency detected - consider adjusting quality thresholds or correction strategies")
...
class CRAGIntegrationFactory:
    ...
    @staticmethod
    def create_crag_system(model_config: ModelConfigManager,
                          document_service: DocumentService,
                          metrics_service: MetricsService,
                          config: Optional[CRAGConfiguration] = None) -> SelfCorrectiveRAG:
    ...
    @staticmethod
    def create_document_processor(crag_system: SelfCorrectiveRAG,
                                document_service: DocumentService,
                                metrics_service: MetricsService,
                                config: Optional[CRAGConfiguration] = None) -> CRAGDocumentProcessor:
        """Create a CRAG document processor"""

        if config is None:
            config = CRAGConfiguration()

        return CRAGDocumentProcessor(crag_system, document_service, metrics_service, config)

    @staticmethod
    def create_webhook_processor(crag_processor: CRAGDocumentProcessor,
                               agent_supervisor: MultiAgentSupervisor) -> CRAGWebhookProcessor:
        """Create a CRAG webhook processor"""

        return CRAGWebhookProcessor(crag_processor, agent_supervisor)

    @staticmethod
    def create_performance_analyzer(metrics_service: MetricsService) -> CRAGPerformanceAnalyzer:

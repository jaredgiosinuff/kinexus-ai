...
JIRA_CONFIG = {
    ...
    'documentation_labels': ['needs-docs', 'documentation', 'api-change', 'breaking-change', 'new-feature'],
...
def should_process_ticket(issue_data: Dict[str, Any], changelog: Dict[str, Any]) -> tuple[bool, str]:
    """
    ...
    if resolution_name in ['Won\'t Fix', 'Won\'t Do', 'Duplicate', 'Cannot Reproduce', 'Invalid']:
    ...
    if issue_type in JIRA_CONFIG['skip_types']:
        ...
        if issue_type == 'Bug':
            if any(label in ['breaking-change', 'api-change', 'documentation'] for label in labels):
...
def extract_documentation_context(issue_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for comment in comments[-5:]:  # Last 5 comments
        ...
        if any(word in body.lower() for word in ['documentation', 'docs', 'readme', 'api']):
            doc_comments.append({
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Received Jira event",
        ...
        if not should_process:
            ...
            return {
                'statusCode': 200,
                'body': json.dumps({
                    'message': 'Ticket received but not processed',
                    'reason': reason
                })
            }
        ...
        logger.info(f"Jira change recorded",
                   change_id=change_id,
                   documentation_type=doc_context['documentation_type'])

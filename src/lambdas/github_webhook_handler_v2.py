...
async def assess_documentation_impact(change_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        response = bedrock.invoke_model(
            ...
            body=json.dumps({"prompt": prompt, "max_tokens": 1000, "temperature": 0.3}),
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            document_data = {
                ...
                "doc_metadata": {"repository": repository_name, "auto_managed": True},
            }
...
async def create_review_via_api(
    ...
    try:
        review_data = {
            ...
            "priority": {"high": 8, "medium": 5, "low": 2}.get(
                ai_assessment.get("priority", "medium"), 5
            ),
        }
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 201:
                ...
                logger.error(
                    f"Failed to create review: {response.status_code} - {response.text}"
                )
            ...
            try:
                ...
                for doc_type in ai_assessment.get("affected_docs", ["general"]):
                    document_id = await find_or_create_document(
                        change_data["repository_name"], doc_type
                    )
                ...
                change_record["status"] = (
                    "reviews_created" if reviews_created else "no_reviews_needed"
                )

                table.put_item(Item=change_record)

                logger.info(
                    f"Created {len(reviews_created)} reviews for change {change_id}"
                )

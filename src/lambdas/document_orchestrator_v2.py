...
class EnhancedDocumentOrchestrator:
    ...
    async def process_github_change(
        ...
        # Check what changed
        has_api_changes = any(
            ".py" in f or ".js" in f or ".ts" in f for f in files_changed
        )
        has_readme_change = any("README" in f.upper() for f in files_changed)
        has_config_change = any(
            "config" in f.lower() or ".env" in f or ".yml" in f for f in files_changed
        )
    ...
    async def generate_github_documentation(
        ...
        try:
            ...
            document_id = (
                f"github_{repository}_{doc_type}_{datetime.utcnow().timestamp()}"
            )
            ...
            return {"document_id": document_id, "s3_key": s3_key, "doc_type": doc_type}
    ...
    async def generate_feature_documentation(
        self, jira_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            # Store metadata
            document_id = (
                f"jira_{jira_context['issue_key']}_{datetime.utcnow().timestamp()}"
            )
            self.documents_table.put_item(
                Item={
                    "document_id": document_id,
            ...
            return {"document_id": document_id, "s3_key": s3_key, "doc_type": "feature"}

        except Exception as e:
            logger.error(f"Error generating feature docs: {str(e)}")
            return {"error": str(e)}

    async def find_related_code_changes(
        self, jira_context: Dict[str, Any]
    ) -> List[Dict]:
        ...
        response = self.changes_table.scan(
            ...
            ExpressionAttributeNames={"#source": "source", "#data": "raw_event"},
            ExpressionAttributeValues={":source": "github", ":issue_key": issue_key},
            Limit=10,
        )
                    ...
                    related_changes.append(
                        {
                            ...
                            "repository": item.get("change_data", {}).get(
                                "repository_name"
                            ),
                        }
                    )
    ...
    def _build_github_prompt(
        self, repository: str, files: List[str], commits: List[Dict], doc_type: str
    ) -> str:

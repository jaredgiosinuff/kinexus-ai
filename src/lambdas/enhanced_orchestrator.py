...
class PlatformOrchestrator:
    ...
    def _init_clients(self) -> Dict:
        ...
        try:
            ...
            from integrations.confluence_client import ConfluenceClient, ConfluenceDocumentManager
            from integrations.sharepoint_client import SharePointClient, SharePointDocumentManager
            ...
            if all([confluence_url, confluence_user, confluence_token]):
                client = ConfluenceClient(confluence_url, confluence_user, confluence_token)
                clients['confluence'] = {
                    'client': client,
    ...
    async def analyze_impact(self, change_data: Dict) -> Dict:
        ...
        context = {
            ...
            'files_changed': change_data.get('change_data', {}).get('files_changed', [])[:20],
            'commits': [c['message'] for c in change_data.get('change_data', {}).get('commits', [])[:10]],
        ...
        try:
            ...
            import re
            json_match = re.search(r'\{.*\}', result.get('content', [{}])[0].get('text', ''), re.DOTALL)
            if json_match:
    ...
    async def find_affected_documents(self, change_data: Dict, impact: Dict) -> List[Dict]:
        """
    ...
    async def _find_github_docs(self, change_data: Dict, keywords: List[str]) -> List[Dict]:
        ...
        doc_files = ['README.md', 'CONTRIBUTING.md', 'docs/API.md', 'docs/Configuration.md']

        for file_path in doc_files:
            try:
                file_data = await self.clients['github']['client'].get_file(repo, file_path)
                if file_data:
                    docs.append({
                        'platform': 'github',
                        'repo': repo,
    ...
    async def generate_updates(self, change_data: Dict, affected_docs: List[Dict]) -> List[Dict]:
        """

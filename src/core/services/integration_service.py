...
from ..models.integrations import (
    ...
    IntegrationType, IntegrationStatus, SyncDirection, AuthType,
    IntegrationCreateRequest, IntegrationUpdateRequest, IntegrationTestResponse,
    INTEGRATION_SCHEMAS, INTEGRATION_CAPABILITIES
)
...
class IntegrationService:
    ...
    def _register_integration_classes(self) -> Dict[IntegrationType, Type[BaseIntegration]]:
    ...
    async def toggle_integration(self, integration_id: str, enabled: bool) -> bool:
        ...
        try:
            ...
            new_status = IntegrationStatus.ACTIVE if enabled else IntegrationStatus.INACTIVE

            await self.integration_repo.update_integration(
                integration_id,
                {"status": new_status, "sync_enabled": enabled}
            )
    ...
    async def test_integration(self, integration_id: str) -> IntegrationTestResponse:
        ...
        try:
            ...
            integration_class = self._integration_classes.get(IntegrationType(integration.integration_type))
    ...
    async def sync_integration(
        ...
        try:
            ...
            integration_class = self._integration_classes.get(IntegrationType(integration.integration_type))
            if not integration_class:
                raise ValueError(f"Integration type {integration.integration_type} not supported")
    ...
    async def _perform_sync(self, integration_instance: BaseIntegration, sync_log_id: str):
        ...
        try:
            ...
            await self.integration_repo.update_sync_log(sync_log_id, {
            ...
            await self.integration_repo.update_integration(
                ...
                {
                    "last_sync": end_time,
                    "next_sync": end_time + timedelta(seconds=integration_instance.integration.sync_frequency),
                    "error_count": 0 if result.success else integration_instance.integration.error_count + 1,
    ...
    async def handle_webhook(
        ...
        try:
            integration = await self.get_integration(integration_id)
            if not integration:
                logger.warning("Webhook received for unknown integration", {
                    "integration_id": integration_id
                })
                return False

            if integration.status != IntegrationStatus.ACTIVE:
                logger.warning("Webhook received for inactive integration", {
                    "integration_id": integration_id,
            ...
            integration_class = self._integration_classes.get(IntegrationType(integration.integration_type))
            if not integration_class:
                logger.error("Unsupported integration type for webhook", {
                    "integration_id": integration_id,
                    "type": integration.integration_type
                })
                return False
            ...
            asyncio.create_task(
                self._process_webhook(integration_instance, delivery.id, event_type, payload)
            )
    ...
    async def _process_webhook(
        ...
        try:
            ...
            await self.integration_repo.update_webhook_delivery(delivery_id, {
    ...
    async def get_integration_stats(self, integration_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def schedule_syncs(self):
        ...
        try:
            ...
            for integration in active_integrations:
                if (integration.sync_enabled and
                    integration.next_sync and
                    integration.next_sync <= current_time):
    ...
    async def cleanup_old_logs(self, days_to_keep: int = 30):
        ...
        try:
            ...
            deleted_logs = await self.integration_repo.cleanup_old_sync_logs(cutoff_date)
            deleted_deliveries = await self.integration_repo.cleanup_old_webhook_deliveries(cutoff_date)
    ...
    async def retry_failed_webhooks(self):
        ...
        try:
            failed_deliveries = await self.integration_repo.get_failed_webhook_deliveries()
            ...
            for delivery in failed_deliveries:
                if delivery.attempts < delivery.max_attempts:
                    ...
                    if integration and integration.status == IntegrationStatus.ACTIVE:
                        ...
                        await self.integration_repo.update_webhook_delivery(delivery.id, {

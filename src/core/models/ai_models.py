...
class BaseModelProvider(ABC):
    ...
    async def generate_response(
        self, model_type: AIModelType, request: ModelRequest
    ) -> ModelResponse:
    ...
    async def generate_response(
        ...
        try:
            ...
            return ModelResponse(
                content=content,
                metadata={"model_id": model_id, "bedrock_response": response_body},
                usage=usage,
...
class ModelManager:
    ...
    def get_model_info(self, model_type: AIModelType) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": [
                cap.value
                for cap in provider_instance.get_model_capabilities(model_type)
            ],
            "limits": provider_instance.get_model_limits(model_type).__dict__,
        }

    def get_optimal_model_for_task(
        self, task_type: str, capabilities_needed: List[ModelCapability] = None
    ) -> AIModelType:
    ...
    def get_provider_status(self) -> Dict[ModelProvider, Dict[str, Any]]:
        ...
        for provider_type, provider in self.providers.items():
            status[provider_type] = {
                "available": True,
                "supported_models": [
                    model.value for model in provider.get_supported_models()
                ],

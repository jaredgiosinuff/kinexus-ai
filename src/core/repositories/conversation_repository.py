...
from ..models.conversations import (
    AgentConversation, ConversationStatus, ConversationFilter,
    ConversationCreate, ConversationUpdate, ConversationSummary,
    ConversationAnalytics, ReasoningStep, ModelCall, PerformanceMetrics,
    LiveConversationStatus, ConversationMetrics
)
...
class ConversationRepository:
    ...
    async def create_conversation(self, conversation_data: ConversationCreate) -> AgentConversation:
            ...
            for field, value in update_dict.items():
                if hasattr(conversation, field):
                    if field == "reasoning_steps" and value:
                        ...
                        setattr(conversation, field, [step.dict() if hasattr(step, 'dict') else step for step in value])
                    elif field == "model_calls" and value:
                        setattr(conversation, field, [call.dict() if hasattr(call, 'dict') else call for call in value])
                    elif field == "performance_metrics" and value:
                        setattr(conversation, field, value.dict() if hasattr(value, 'dict') else value)
                    else:
                        setattr(conversation, field, value)

            # Update timestamps based on status changes
            if update_data.status:
                if update_data.status == ConversationStatus.RUNNING and not conversation.started_at:
                    conversation.started_at = datetime.utcnow()
                elif update_data.status in [ConversationStatus.COMPLETED, ConversationStatus.FAILED, ConversationStatus.CANCELLED]:
                    if not conversation.completed_at:
                        conversation.completed_at = datetime.utcnow()
                    if conversation.started_at:
                        conversation.duration_seconds = (conversation.completed_at - conversation.started_at).total_seconds()
    ...
    async def get_active_conversations(self) -> List[AgentConversation]:
        ...
        try:
            ...
            ).order_by(desc(AgentConversation.started_at)).all()

            logger.debug("Active conversations retrieved", {"count": len(conversations)})
    ...
    async def get_conversation_summary(
        ...
        try:
            ...
            aggregates = self.db.query(
                ...
                func.avg(AgentConversation.confidence_score).label('avg_confidence'),
    ...
    async def get_conversations_by_filter(self, filter_criteria: ConversationFilter) -> List[AgentConversation]:
        ...
        try:
            ...
            if filter_criteria.agent_type:
                query = query.filter(AgentConversation.agent_type == filter_criteria.agent_type)

            if filter_criteria.status:
                query = query.filter(AgentConversation.status == filter_criteria.status)

            if filter_criteria.start_date:
                query = query.filter(AgentConversation.created_at >= filter_criteria.start_date)

            if filter_criteria.end_date:
                query = query.filter(AgentConversation.created_at <= filter_criteria.end_date)

            if filter_criteria.min_confidence:
                query = query.filter(AgentConversation.confidence_score >= filter_criteria.min_confidence)

            if filter_criteria.max_cost:
                query = query.filter(AgentConversation.total_cost <= filter_criteria.max_cost)

            if filter_criteria.reasoning_pattern:
                query = query.filter(AgentConversation.reasoning_pattern == filter_criteria.reasoning_pattern)
            ...
            logger.debug("Filtered conversations retrieved", {
                "count": len(conversations),
    ...
    async def get_conversation_analytics(
        ...
        try:
            ...
            if agent_type:
                base_query = base_query.filter(AgentConversation.agent_type == agent_type)
            ...
            if agent_type:
                volume_query = volume_query.filter(AgentConversation.agent_type == agent_type)
            ...
            total_completed = base_query.filter(
                AgentConversation.status.in_([ConversationStatus.COMPLETED, ConversationStatus.FAILED])
            ).count()
            successful = base_query.filter(AgentConversation.status == ConversationStatus.COMPLETED).count()
            ...
            if agent_type:
                cost_query = cost_query.filter(AgentConversation.agent_type == agent_type)

            cost_data = cost_query.group_by(AgentConversation.model_used).all()
            cost_breakdown = {row.model_used: float(row.total_cost) for row in cost_data}
            ...
            if agent_type:
                model_query = model_query.filter(AgentConversation.agent_type == agent_type)
            ...
            if agent_type:
                pattern_query = pattern_query.filter(AgentConversation.agent_type == agent_type)

            pattern_data = pattern_query.group_by(AgentConversation.reasoning_pattern).all()
            reasoning_pattern_usage = {row.reasoning_pattern: row.usage_count for row in pattern_data}
            ...
            if agent_type:
                error_query = error_query.filter(AgentConversation.agent_type == agent_type)

            error_data = error_query.group_by(AgentConversation.error_message).order_by(
                desc('error_count')
            ).limit(10).all()
            ...
            logger.info("Conversation analytics generated", {
                "time_period": time_period,
    ...
    async def get_live_conversation_status(self, conversation_id: str) -> Optional[LiveConversationStatus]:
        ...
        try:
            ...
            current_step = reasoning_steps[-1]['thought'] if reasoning_steps else "Starting..."
            ...
            progress_percentage = min((current_steps / estimated_total_steps) * 100, 95)  # Cap at 95% until complete

            # Estimate completion time based on average step duration
            if len(reasoning_steps) > 1:
                avg_step_duration = sum(step.get('duration_ms', 0) for step in reasoning_steps) / len(reasoning_steps)
                remaining_steps = max(estimated_total_steps - current_steps, 1)
                estimated_completion = datetime.utcnow() + timedelta(milliseconds=avg_step_duration * remaining_steps)
            else:
            ...
            logger.error("Failed to get live conversation status", {
                "conversation_id": conversation_id,
                "error": str(e)
            })
            raise
    ...
    async def get_conversation_metrics(self) -> ConversationMetrics:
        ...
        try:
            ...
            active_count = self.db.query(AgentConversation).filter(
                AgentConversation.status == ConversationStatus.RUNNING
            ).count()

            # Queue length (pending conversations)
            queue_length = self.db.query(AgentConversation).filter(
                AgentConversation.status == ConversationStatus.PENDING
            ).count()

            # Average wait time (time between creation and start)
            wait_time_result = self.db.query(
                func.avg(
                ...
                and_(
                    ...
                    AgentConversation.created_at >= datetime.utcnow() - timedelta(hours=24)
                )
            ).scalar()
            ...
            throughput_per_hour = self.db.query(AgentConversation).filter(
                and_(
                    AgentConversation.status == ConversationStatus.COMPLETED,
            ...
            total_completed_24h = self.db.query(AgentConversation).filter(
                and_(
                    AgentConversation.completed_at >= twenty_four_hours_ago,
                    AgentConversation.status.in_([ConversationStatus.COMPLETED, ConversationStatus.FAILED])
                )
            ).count()
            successful_24h = self.db.query(AgentConversation).filter(
                and_(
                    AgentConversation.completed_at >= twenty_four_hours_ago,
                    AgentConversation.status == ConversationStatus.COMPLETED
                )
            ).count()
            success_rate_24h = (successful_24h / total_completed_24h) if total_completed_24h > 0 else 0

            # Cost per hour
            cost_per_hour_result = self.db.query(
                func.sum(AgentConversation.total_cost)
            ).filter(
                and_(
                    AgentConversation.completed_at >= one_hour_ago,
                    AgentConversation.status == ConversationStatus.COMPLETED
            ...
            top_agents_result = self.db.query(
                AgentConversation.agent_type,
                func.count(AgentConversation.id).label('count')
            ).filter(
                AgentConversation.created_at >= twenty_four_hours_ago
            ).group_by(AgentConversation.agent_type).order_by(desc('count')).limit(5).all()
            ...
            pattern_performance_result = self.db.query(
                AgentConversation.reasoning_pattern,
                func.avg(AgentConversation.duration_seconds).label('avg_duration'),
                func.avg(AgentConversation.confidence_score).label('avg_confidence'),
            ...
            ).group_by(AgentConversation.reasoning_pattern).all()
            ...
            logger.debug("Conversation metrics calculated", {
                "active_count": active_count,
                "queue_length": queue_length,
                "success_rate_24h": success_rate_24h
            })

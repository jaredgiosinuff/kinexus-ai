...
23. Reviewer posts comment on `TOAST-43`:
...
24. Jira webhook fires (comment added) → `approval_handler` Lambda
25. **Robust Review Ticket Detection** (two-method approach):
...
26. **ADF Text Extraction** (Jira REST API v3):
...
27. **Document ID Extraction with Timestamp Support:**
...
28. **Parses comment** for approval decision:

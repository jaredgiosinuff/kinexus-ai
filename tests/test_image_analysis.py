...
from src.agents.image_analysis_framework import (
    ...
    ImageType, AnalysisTask, ImagePreprocessor, TextExtractor,
    VisualQualityAnalyzer, ContentValidator
)
from src.agents.image_analysis_integration import (
    CRAGImageEnhancer, DocumentationImageValidator,
...
class TestVisualQualityAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_assess_quality_high_quality(self, quality_analyzer, high_quality_image):
    ...
    async def test_assess_readability(self, quality_analyzer, high_quality_image):
        ...
        readability_score = await quality_analyzer._assess_readability(high_quality_image)
    ...
    async def test_assess_accessibility(self, quality_analyzer, high_quality_image):
        ...
        accessibility_score = await quality_analyzer._assess_accessibility(high_quality_image)
...
class TestContentValidator:
    ...
    async def test_validate_text_content(self, content_validator):
        ...
        result = await content_validator._validate_text_content(extracted_text, required_text)
...
class TestImageAnalysisEngine:
    ...
    async def test_analyze_image_basic(self, analysis_engine, sample_request):
        ...
        with patch.object(analysis_engine.text_extractor, 'extract_text') as mock_text:
            ...
            with patch.object(analysis_engine.quality_analyzer, 'assess_quality') as mock_quality:
    ...
    async def test_analyze_image_error_handling(self, analysis_engine, sample_request):
        ...
        with patch.object(analysis_engine.preprocessor, 'preprocess_image') as mock_preprocess:
...
class TestImageAnalysisIntegration:
    ...
    async def test_health_check(self, integration_manager):
        ...
        with patch.object(integration_manager.image_analyzer, 'analyze_image') as mock_analyze:
            mock_analyze.return_value = Mock(
                confidence=0.8,
                processing_time=1.0
            )
...
class TestDocumentationImageValidator:
    ...
    def test_classify_image_from_context(self, image_validator):
        ...
        img_type = image_validator._classify_image_from_context("Screenshot of login", "")
        ...
        img_type = image_validator._classify_image_from_context("System architecture", "")
...
class TestCRAGImageEnhancer:
    ...
    async def test_process_query_with_images(self, crag_enhancer):
        ...
        crag_enhancer.image_analyzer.batch_analyze_images.return_value = [mock_image_result]

        # Mock enhanced response generation
        with patch.object(crag_enhancer, '_enhance_with_image_insights') as mock_enhance:
...
class TestImageAnalysisPerformance:
    ...
    async def test_performance_single_image(self, analysis_engine):
        ...
        with patch.object(analysis_engine, '_calculate_overall_confidence', return_value=0.8):
            with patch.object(analysis_engine.quality_analyzer, 'assess_quality') as mock_quality:
